...
import { MemcacheGoPlugin } from '../index.js';

const __filename = url.fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const FIXTURE_ROOT = path.resolve(__dirname, '../../../../tests/fixtures/memcache-go/basic');

async function extract(file: string): Promise<NodeBatch> {
  const plugin = new MemcacheGoPlugin();
...
describe('framework-memcache-go visitor', () => {
  it('emits one DatabaseInteraction per memcache verb', async () => {
    const batch = await extract('cache.go');
    // getUser, setUser, addEntry, replaceEntry, incrCounter,
    // decrCounter, touchKey, deleteSession, flushAll = 9
    // dynamicKey skipped
    expect(interactions(batch).length).toBe(9);
  });

  it('extracts key from string args and from Item composite literals', async () => {
    const batch = await extract('cache.go');
  ...
  it('every interaction carries orm="memcache-go"', async () => {
    const batch = await extract('cache.go');
    for (const i of interactions(batch)) expect(i.orm).toBe('memcache-go');
  });
  ...
  it('rejects all emits in a file without gomemcache import', async () => {
