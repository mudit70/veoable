...
import { AzureBlobPyPlugin } from '../index.js';

const FIXTURE_ROOT = path.resolve(__dirname, '../../../../tests/fixtures/azure-blob-py/basic');

async function extract(file: string): Promise<NodeBatch> {
  const plugin = new AzureBlobPyPlugin();
...
describe('framework-azure-blob-py visitor', () => {
  ...
  it('every caller carries framework="azure-blob-py"', async () => {
    const batch = await extract('storage_ops.py');
    for (const c of callers(batch)) {
      expect(c.framework).toBe('azure-blob-py');
    }
  });
