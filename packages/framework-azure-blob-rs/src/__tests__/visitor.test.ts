...
import { AzureBlobRsPlugin } from '../index.js';

const FIXTURE_ROOT = path.resolve(__dirname, '../../../../tests/fixtures/azure-blob-rs/basic');

async function extract(file: string): Promise<NodeBatch> {
  const plugin = new AzureBlobRsPlugin();
  const rust = new RustLanguagePlugin();
  plugin.onProjectLoaded?.({
    rootDir: FIXTURE_ROOT,
    repository: 'azure-blob-rs-fixture',
...
describe('framework-azure-blob-rs visitor', () => {
  ...
  it('maps methods to HTTP verbs', async () => {
    ...
    expect(byMethod.get('PUT')).toBe(4);
    expect(byMethod.get('DELETE')).toBe(2);
  });

  it('every caller carries framework="azure-blob-rs"', async () => {
    const batch = await extract('src/main.rs');
    for (const c of callers(batch)) {
      expect(c.framework).toBe('azure-blob-rs');
    }
  });
  ...
  it('builds azure://<container>/<blob> URLs for blob-scope chains', async () => {
    ...
    expect(urls).toContain('azure://raw-container/raw-key');
  });

  it('builds azure://<container>/ URLs for container-scope chains', async () => {
    const batch = await extract('src/main.rs');
    const urls = callers(batch).map((c) => c.urlLiteral);
    expect(urls).toContain('azure://static-assets/');
  ...
  it('rejects all emits in a file with no azure_storage_blobs use', async () => {
