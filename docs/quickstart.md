...
```sh
adorable project init .                              # generates <name>.project.json
adorable project analyze <name>.project.json --fresh # writes <name>.db
```
...
```sh
claude mcp add --scope project adorable -- adorable serve $(pwd)/<name>.db --project-config $(pwd)/<name>.project.json
```

The `--` separator before `adorable serve` is required so Claude doesn't intercept `--project-config`.
...
> Use the adorable MCP server to give me a high-level architecture overview.
...
```sh
adorable project analyze <name>.project.json --fresh
```
...
- **`claude mcp list` shows "✗ Failed to connect"**: check the path to your `.db` file is absolute, and that you've run `adorable project analyze` at least once to create it.
